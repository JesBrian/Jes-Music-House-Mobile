...
class SearchResultPlayList extends React.Component {
  render() {
    return (
      ...
      <View style={{height:53, flexDirection:'row'}}>
        <Ripple onPress={() => {this.props.navigation.navigate('SingerDetail')}} style={{flex:1, flexDirection:'row', alignItems:'center'}}>
          ...
          <View style={{height:'100%', flex:1, justifyContent:'center', borderBottomWidth:0.18, borderColor:'#888'}}>
            <Text style={{marginTop:5, marginBottom:3, fontSize:16, color:'#EEE'}}>搜索的歌单</Text>
            <Text style={{fontSize:13, color:'#AAA'}}>180首 by JesBrian</Text>
          </View>
        </Ripple>
      </View>
    );
...
export default withNavigation(SearchResultPlayList);
